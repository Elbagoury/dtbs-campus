...
class Filter(models.Model):
    ...
    @api.multi
    def onchange_study(self, study, tahun, faculty, department, kuota):
        if not study:
            return {"value": {"filterlist_ids": []}}
		
        reg = self.env['res.partner'].search([("is_registran", "=", True), ("tahun", "=",tahun), ("faculty_id", "=", faculty), ("department_id", "=", department), ("study_id", "=", study), ("sts_academic", "=", "calon")], limit=kuota, order="nilai desc")

        return {'value':{'filterlist_ids':reg}}
        ...
        for obj in self.browse(self.ids):
            ...
            if len(list1) <=0:
                ...
                for fieldrec in obj.filterlist_ids:
                    self.env["res.partner"].search([("id", "=", fieldrec.id)]).write({"sts_academic": "siswa"})
        self.state = "done"
